...
The subsystem marks itself healthy by issuing `XPm_InitFinalize`. The Linux system, when it completes all the boot activities, calls `XPm_InitFinalize` to finalize its PM requirements. When Linux fails (or is not allowed) to boot, this never called and hence PLM will trigger the recovery.
