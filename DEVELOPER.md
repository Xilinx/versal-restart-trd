...
* With the help of `DRMsg Protocol`, a custom message sharing over DDR Memory, TRD can perform several tasks!
  * A slave (eg: APU) polls it's specific region in DDR space, waiting to perform necessary action. The application `*-app.py` contains this algorithm.
...
* The generated .rpm package can be installed on both APU as well as System Controller.
  ...
  * With this detection mechanism, the package configures and installs the necessary components accordingly.
