...
1. RPU application triggers apu power down by calling XilPm client API `XPm_ForcePowerDown`, with APU subsystem id as arugment

2. After that we try to wakeup APU subsystem by loading partial PDI through an IPI command from Image store
...
After that the API `XIpiPsu_TriggerIpi` triggers subsystems loading from the DDR address. PLM receives ipi call, reloads the subsystem from the DDR address and the subsystem requests necessary nodes.
